...
public class Picture extends SimplePicture {
	...
	public void mirrorHorizontalBotToTop() {
		...
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < width; col++) {
				leftPixel = pixels[getHeight() - 1 - row][col];
				rightPixel = pixels[row][col];
				rightPixel.setColor(leftPixel.getColor());
			}
		}
	...
	public void mirrorGull() {
		...
		for (int row = 235; row < 319; row++) {
			...
			for (int col = 235; col < 345; col++) {
				count++;
				leftPixel = pixels[row][col];
				rightPixel = pixels[row+20][col+ 150];
				rightPixel.setColor(leftPixel.getColor());
			}
		}
		System.out.println(count);
	...
	public void mirrorArms() {
		...
		for (int row = 159; row < 190; row++) {
			...
			for (int col = 105; col < mirrorPoint; col++) {
				...
				leftPixel = pixels[row][col];
				rightPixel = pixels[row + (250-179)][col - (7)];
				rightPixel.setColor(leftPixel.getColor());
			}
		}
	...
	public void secondCopy(Picture fromPic, int startRow, int startCol, int endRow, int endCol) {
		Pixel fromPixel = null;
		Pixel toPixel = null;
		Pixel[][] toPixels = this.getPixels2D();
		Pixel[][] fromPixels = fromPic.getPixels2D();
	...
	public void myCollage(Picture one, Picture two, Picture three) {
		...
		secondCopy(one, 25, 25, 50, 50);
		secondCopy(two, 100, 100, 200, 200);
		secondCopy(three, 0, 0, 40, 40);
